...
To build BlabApp [Maven](https://maven.apache.org) is required.

`mvn package` will build the web application and output a war file to `target/blabapp.war`.

## Configure

To run BlabApp [MySQL](https://www.mysql.com/) and [Tomcat](https://tomcat.apache.org/) are required.
...
BlabApp will also run out-of-the-box in AWS. Simply upload the `target/blabapp.war` file into a Tomcat Elastic Beanstalk environment (with associated Amazon RDS). The database credentials listed above are not required when running in AWS.
